...
class TestCollectionEnvironmentVariables:
            ...
            (
                "tests/env_files/validation_schema/path_issues_env_5",
                {
                    ...
                    "validation_folder": str(
                        Path().resolve() / "test_engine_app/validation_schemas"
                    ),
                    "consumer_group": "MyGroup123",
                    "hostname": "192.168.1.1",
                    "server_port": 1234,
    ...
    def test_init(self, env_location, expected_result, expected_print_output):
        ...
        # Assert
        assert (
            EnvironmentVariables.get_core_modules_folder()
            == expected_result["core_modules_folder"]
        )
        assert (
            EnvironmentVariables.get_validation_schemas_folder()
            == expected_result["validation_folder"]
        )
        assert (
            EnvironmentVariables.get_redis_consumer_group()
            == expected_result["consumer_group"]
        )
        assert (
        ...
        assert (
            ...
            == expected_result["server_port"]
        )
        assert (
            EnvironmentVariables.get_api_server_port()
            == expected_result["api_server_port"]
        )
        assert (
            EnvironmentVariables.print_environment_variables() == expected_print_output
        )
