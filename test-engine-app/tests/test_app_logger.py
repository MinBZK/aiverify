...
class TestCollectionAppLogger:
    ...
    def test_generate_logger(self):
        ...
        assert (
            new_applogger.raw_logger_instance
            == new_applogger.logger_instance.get_logger()
        )
        assert new_applogger.raw_logger_instance.getEffectiveLevel() == logging.DEBUG
        assert (
            new_applogger.log_filepath
            == new_applogger.logger_instance.get_log_filepath()
        )

        assert new_applogger.error_logger_instance is not None
        assert isinstance(new_applogger.error_logger_instance, ErrorManager)
        assert (
            new_applogger.error_filepath
            == new_applogger.error_logger_instance.get_error_filepath()
        )
    ...
    def test_add_error_to_log(
        self, app_logger, category, code, description, severity, component
    ):
        """
    ...
    def test_get_errors_in_json_str(self, mocker, app_logger, expected_output):
        ...
        with mocker.patch.object(
            ErrorManager, "get_errors_as_json_string", return_value="HelloWorld"
        ):
    ...
    @pytest.mark.parametrize(
        ...
        [
            (pytest.my_logger, True, True),
            (None, True, False),
            ("None", True, False),
            ({}, True, False),
            ([], True, False),
            (1234, True, False),
            (pytest.my_logger, False, False),
            (None, False, False),
            ("None", False, False),
            ({}, False, False),
            ([], False, False),
    ...
    def test_write_error_to_file(
        self, mocker, app_logger, return_value, expected_output
    ):
        """
        Tests writing error to file
        """
        with mocker.patch.object(
            ErrorManager, "write_error_to_file", return_value=return_value
        ):
