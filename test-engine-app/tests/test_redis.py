...
class TestCollectionRedis:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                ...
                (
                    False,
                    "The middleware is unreachable. There was a connection error to the redis server: "
    ...
    def test_connect_to_stream(
        ...
        # Connect to stream
        output_tuple = Redis.connect_to_stream(
    ...
    def test_connect_to_stream_no_redis_server(
        self,
        hostname,
        hostport,
        consumer_group,
    ...
    def test_connect_to_pubsub_mock_failed_subscription(
        self, mocker, kwargs, expected_output
    ):
        """
    ...
    @pytest.mark.parametrize(
        ...
        [
            ("partial_dependence_plot", {"partial_dependence_plot": "algorithm_info"}),
            ("", {}),
        ],
    )
    def test_get_algorithm_info(self, mocker, algorithm_id, expected_output):
        ...
        """
        with mocker.patch.object(
            HashCommands, "hgetall", return_value={algorithm_id: "algorithm_info"}
        ):
            # Set logger
            Redis.set_logger(pytest.my_logger)
    ...
    def test_get_algorithm_info_no_stream_listener(
        self, mocker, algorithm_id, expected_output
    ):
        """
        Tests getting the algorithm info
        """
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            ("partial_dependence_plot", {}, {}),
            ("partial_dependence_plot", [], {}),
            ("partial_dependence_plot", 1234, {}),
        ],
    )
    def test_get_algorithm_info_invalid_read_algorithm_registry(
        self, mocker, algorithm_id, return_value, expected_output
    ):
        """
        Tests getting the algorithm info
        """
        with mocker.patch.object(
            RedisStream, "read_algorithm_registry", return_value=return_value
        ):
            # Set logger
            Redis.set_logger(pytest.my_logger)
    ...
    def test_get_algorithm_info_connection_error(
        self, mocker, algorithm_id, return_value, expected_output
    ):
        """
        Tests getting the algorithm info
    ...
    @pytest.mark.parametrize(
        ...
        [
            (1, 1000, True, ["mymockmessage"]),
            (None, 1000, True, []),
            ("None", 1000, True, []),
            ({}, 1000, True, []),
            ([], 1000, True, []),
            (1234, 1000, True, ["mymockmessage"]),
            (1, None, True, ["mymockmessage"]),
            (1, "None", True, []),
            (1, {}, True, []),
            (1, [], True, []),
            (1, 1234, False, []),
    ...
    def test_get_new_items(
        ...
        """
        with mocker.patch.object(
            StreamCommands, "xreadgroup", return_value=["mymockmessage"]
        ):
            # Set logger
            Redis.set_logger(pytest.my_logger)
            ...
            if true_callback_function:
                output = Redis.get_new_items(
                    self.my_callback_function_get_items,
                    read_num_of_jobs,
                    blocking_duration_ms,
                )
            else:
                output = Redis.get_new_items(
                    self.my_callback_function_false_get_items,
                    read_num_of_jobs,
                    blocking_duration_ms,
                )
    ...
    def test_get_new_items_invalid_validation(
        self, mocker, read_num_of_jobs, blocking_duration_ms, expected_output
    ):
        """
        Tests getting the new items
        """
        with mocker.patch.object(
            ...
            # Get the algorithm information
            output = Redis.get_new_items(None, read_num_of_jobs, blocking_duration_ms)
    ...
    def test_get_new_items_no_stream_listener(
        self, mocker, read_num_of_jobs, blocking_duration_ms, expected_output
    ):
        """
        Tests getting the new items
    ...
    def test_get_new_items_invalid_read_message(
        self, mocker, read_num_of_jobs, blocking_duration_ms, expected_output
    ):
        """
        Tests getting the new items
    ...
    def test_get_new_items_connection_error(
        self, mocker, read_num_of_jobs, blocking_duration_ms, expected_output
    ):
        """
        Tests getting the new items
        """
        with mocker.patch.object(
            RedisStream, "read_message", side_effect=self.connection_error_side_effect
        ):
            # Set logger
            Redis.set_logger(pytest.my_logger)
    ...
    def test_get_pending_items(self, mocker, true_callback_function, expected_output):
        ...
        """
        with mocker.patch.object(
            RedisStream, "read_number_of_pending_messages", return_value=1
        ), mocker.patch.object(
            StreamCommands, "xreadgroup", return_value=["mymockmessage"]
        ):
            # Set logger
            ...
            if true_callback_function:
                ...
                output = Redis.get_pending_items(
                    self.my_callback_function_false_get_items
                )
    ...
    def test_get_pending_items_no_pending_items(
        self, mocker, true_callback_function, expected_output
    ):
        """
        Tests getting the pending items
        """
        with mocker.patch.object(
            RedisStream, "read_number_of_pending_messages", return_value=0
        ), mocker.patch.object(
            StreamCommands, "xreadgroup", return_value=["mymockmessage"]
        ):
            # Set logger
    ...
    def test_get_pending_items_connection_error(
        self, mocker, true_callback_function, expected_output
    ):
        """
        Tests getting the pending items
    ...
    def test_get_pending_items_invalid_pending_message(
        self, mocker, true_callback_function, expected_output
    ):
        """
        Tests getting the pending items
    ...
    def test_get_pending_items_no_stream_listener(
        self, mocker, true_callback_function, expected_output
    ):
        """
        Tests getting the pending items
    ...
    def test_send_message_acknowledgement_no_stream_listener(
        self, mocker, message_id, expected_output
    ):
        """
        Tests sending message acknowledgement
        """
    ...
    def test_send_message_acknowledgement_connection_error(
        self, mocker, message_id, expected_output
    ):
        """
        Tests sending message acknowledgement
    ...
    @pytest.mark.parametrize(
        "update_id, update_dict, expected_output",
        [
            ("myupdateid", {"response": "good"}, False),
            (None, {}, False),
            ("None", {}, False),
            ({}, {}, False),
            ([], {}, False),
    ...
    def test_send_update_no_stream_listener(
        self, mocker, update_id, update_dict, expected_output
    ):
        """
        Tests sending update
        """
    ...
    def test_send_update_connection_error(
        self, mocker, update_id, update_dict, expected_output
    ):
        """
        Tests sending message acknowledgement
        """
        with mocker.patch.object(
            RedisStream, "send_update", side_effect=self.connection_error_side_effect
        ):
            # Set logger
            Redis.set_logger(pytest.my_logger)
